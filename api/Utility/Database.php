...
class Database {
    ...
    public function updateInfo($table, $id, array $fields) {
        if (count($fields)) {
            $x = 1;
            $set = "";
            $params = [];
            ...
            if (!$this->query("UPDATE `{$table}` SET {$set} WHERE `user_id` = {$id}", $params)->error()) {
                return true;
            }
        }
        return false;
    ...
    public function updateCompany($id, array $fields) {
        if (count($fields)) {
            $x = 1;
            $set = "";
            $params = [];
            ...
            if (!$this->query("UPDATE company SET {$set} WHERE `company_id` = {$id}", $params)->error()) {
                return true;
            }
        }
        return false;
    ...
    public function selectCompany($companyID, $userID) {
        $params = [];
        $params['company_id'] = $companyID;
        $params['user_id'] = $userID;
        if (!$this->query("SELECT * FROM company WHERE `user_id`=$userID AND `company_id`=$companyID;", $params)->error()) {
            return $this;
        }
